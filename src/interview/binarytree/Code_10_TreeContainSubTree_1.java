 ...
 * 给定彼此独立的两棵树头节点分别为 t1 和t2，判断1 中是否有与2 树拓扑结构完全相同的子树。
 */
public class Code_10_TreeContainSubTree_1 {
    ...
    public static int getIndexOf(String str, String match){
        ...
        char[] mathChars = match.toCharArray();
        int strIndex = 0;
        int matchIndex = 0;
        int[] next = getNextArray(mathChars);
        while(strIndex < strChars.length && matchIndex < mathChars.length){
            if(strChars[strIndex] == mathChars[matchIndex]){
                strIndex++;
                matchIndex++;
        ...
        return matchIndex == mathChars.length ? strIndex - matchIndex : -1;
    }
